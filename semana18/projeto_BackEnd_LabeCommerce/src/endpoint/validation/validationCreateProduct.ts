export default function validationCreateProduct(actionType: "new" | "edit", body: any) {
    const expectedObject: Array<string> = ["name", "description", "price"]
    ...
    const checkers: any = {
        ...
        isValidProductDescription: (input: any): boolean => {
            if (!input.trim().length || input.length > 255) {
                errorTips.push("'description' is empty or longer than 255 characters")
                return false
            } else {
                return true
            }
    ...
    const expectedValues: any = {
        ...
        description: (input: any) => checkers.isValidProductDescription(input),
